...
function Metric({ label, value, note, help }: { label: string; value: string; note: string; help?: string }) {
  return <article className="detail-metric"><p>{label}{help && <MetricHelp text={help} />}</p><strong>{value}</strong><span>{note}</span></article>;
}
  ...
  return <div className="app-shell">
    ...
    <div className="content"><header className="topbar"><button className="icon-button mobile-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Mở trình đơn"><Menu size={20} /></button><Link className="back-link" href="/websites"><ArrowLeft size={17} />Danh sách website</Link><div className="user-area"><button type="button" className="icon-button theme-toggle" onClick={toggleTheme} aria-label={theme === "dark" ? "Chuyển sang chế độ sáng" : "Chuyển sang chế độ tối"} title={theme === "dark" ? "Chế độ sáng" : "Chế độ tối"}>{theme === "dark" ? <Sun size={18} /> : <Moon size={18} />}</button><NotificationBell /><span className="avatar"><CircleUserRound size={21} /></span><span><strong>Quản trị viên</strong><small>Administrator</small></span></div></header>
      <main className="detail-main">
        ...
        <section className="detail-metrics"><Metric label="Uptime 30 ngày" value={website.lastCheckedAt ? `${Number(website.uptimePercent).toFixed(2)}%` : "—"} note="Từ các lần kiểm tra HTTP" help="Tỷ lệ thời gian website hoạt động bình thường trong 30 ngày. Càng gần 100% càng tốt." /><Metric label="Thời gian phản hồi" value={latest.availability?.responseTimeMs ? `${latest.availability.responseTimeMs} ms` : "—"} note={`HTTP ${latest.availability?.details?.statusCode ?? "chưa có"}`} help="Thời gian máy chủ bắt đầu phản hồi yêu cầu. Dưới 800 ms là tốt; số càng thấp website càng phản hồi nhanh." /><Metric label="SSL còn hạn" value={sslDays === null ? "—" : `${sslDays} ngày`} note={website.sslExpiresAt ? `Hết hạn ${formatDate(website.sslExpiresAt)}` : "Chưa có kết quả"} help="Số ngày còn lại trước khi chứng chỉ HTTPS hết hạn. Nên gia hạn trước khi còn dưới 14 ngày." /><Metric label="Lần quét gần nhất" value={website.lastCheckedAt ? "Đã hoàn tất" : "Chưa quét"} note={formatDate(website.lastCheckedAt)} help="Thời điểm hệ thống hoàn tất lần kiểm tra website gần nhất." /></section>
        <section className="detail-grid"><article className="panel performance-panel"><div className="panel-head"><div><h2>PageSpeed Insights <MetricHelp text="Bộ điểm của Google Lighthouse đánh giá tốc độ và trải nghiệm tải trang. Điểm từ 90 là tốt, 50-89 cần cải thiện, dưới 50 là kém." /></h2><p>Kết quả Lighthouse gần nhất</p></div><Gauge size={19} /></div><div className="strategy-grid">{[["Mobile", latest.mobile], ["Desktop", latest.desktop]].map(([name, raw]) => { const check = raw as Check | undefined; return <div className="strategy" key={name as string}><header><strong>{name as string}</strong><span className={`big-score ${(check?.performanceScore ?? 0) >= 90 ? "good" : (check?.performanceScore ?? 0) >= 50 ? "warn" : "bad"}`}>{check?.performanceScore ?? "—"}</span></header><dl><div><dt>FCP <MetricHelp text="Thời gian để nội dung đầu tiên xuất hiện. Dưới 1,8 giây là tốt." /></dt><dd>{check?.details?.fcpSeconds ? `${check.details.fcpSeconds.toFixed(2)}s` : "—"}</dd></div><div><dt>LCP <MetricHelp text="Thời gian để nội dung chính lớn nhất hiển thị. Dưới 2,5 giây là tốt." /></dt><dd>{check?.lcpSeconds ? `${check.lcpSeconds.toFixed(2)}s` : "—"}</dd></div><div><dt>CLS <MetricHelp text="Mức độ bố cục bị nhảy khi tải trang. Dưới 0,1 là tốt; càng gần 0 càng ổn định." /></dt><dd>{check?.clsScore ?? "—"}</dd></div></dl><small>{check ? formatDate(check.checkedAt) : "Chưa có kết quả"}</small></div>; })}</div></article>
        ...
        {wordpress?.connected && <section className="panel seo-posts-panel">
          ...
          {postsSeo && <>
            ...
            <div className="seo-summary">
              ...
              <article className="average"><span>Điểm Rank Math trung bình <MetricHelp text="Điểm SEO trung bình do Rank Math chấm trên thang 100. Từ 80 là tốt, 70-79 khá, dưới 70 nên tối ưu thêm." /></span><strong>{postsSeo.summary.averageScore ?? "—"}</strong><small>{postsSeo.summary.rankMathScored}/{postsSeo.summary.total} bài đã được Rank Math chấm</small></article>
              <article className="attention"><span>Cần tối ưu <MetricHelp text="Số bài viết có điểm Rank Math dưới 70 và nên được kiểm tra lại nội dung SEO." /></span><strong>{postsSeo.summary.needsAttention}</strong><small>Bài có điểm dưới 70</small></article>
        ...
        {wordpress?.connected && <section className="panel malware-panel">
          ...
          {malwareScan && <>
            ...
            <div className="security-summary malware-summary"><article><span>Tổng file <MetricHelp text="Tổng số file mà SiteOps Agent tìm thấy trong phạm vi quét." /></span><strong>{malwareScan.totalFiles}</strong></article><article className="safe"><span>Đã quét <MetricHelp text="Số file đã được đọc và so sánh với các dấu hiệu mã độc." /></span><strong>{malwareScan.scannedFiles}</strong></article><article className="danger"><span>Nguy hiểm <MetricHelp text="File có dấu hiệu rủi ro cao, như mã thực thi trong thư mục uploads. Cần kỹ thuật viên kiểm tra sớm; chưa nên tự động xóa." /></span><strong>{malwareScan.dangerCount}</strong></article><article className="warn"><span>Cảnh báo <MetricHelp text="Dấu hiệu đáng ngờ cần xem thêm nhưng chưa đủ để kết luận là mã độc." /></span><strong>{malwareScan.warningCount}</strong></article><article><span>Thông tin <MetricHelp text="Phát hiện có mức rủi ro thấp, được lưu để theo dõi và đối chiếu." /></span><strong>{malwareScan.infoCount}</strong></article></div>
