<?php
/**
 * Plugin Name: SiteOps Security Agent
...
define( 'SITEOPS_SECURITY_AGENT_VERSION', '1.1.0' );
    ...
    static function () {
        register_rest_route(
            ...
            array(
                ...
                'callback'            => 'siteops_security_agent_scan_plugins',
 ...
 */
function siteops_security_agent_scan_plugins() {
    ...
    foreach ( get_plugins() as $plugin_file => $plugin_data ) {
        $results[] = siteops_security_agent_scan_plugin( $plugin_file, $plugin_data );
    }

    return rest_ensure_response(
        array(
            'agentVersion'     => SITEOPS_SECURITY_AGENT_VERSION,
...
function siteops_security_agent_scan_plugin( $plugin_file, $plugin_data ) {
    ...
    $files      = '.' === $directory
        ...
        : siteops_security_agent_list_files( $plugin_dir );
    $checksums  = siteops_security_agent_get_checksums( $slug, $version );
    $findings   = array();
    $modified   = array();
    $missing    = array();
    $unexpected = array();
    ...
    $code_signals = 'siteops-security-agent' === $slug ? array() : siteops_security_agent_scan_code_signals( $files );
    if ( $modified ) {
    ...
    $risk      = 'low';
    if ( 'modified' === $integrity || siteops_security_agent_has_high_signal( $code_signals ) ) {
        $risk = 'high';
    } elseif ( $code_signals ) {
        $risk = 'medium';
...
function siteops_security_agent_get_checksums( $slug, $version ) {
    if ( ! $slug || ! $version ) {
        return null;
    }
...
function siteops_security_agent_list_files( $root ) {
    $files = array();
    if ( ! is_dir( $root ) ) {
        return $files;
    }
...
function siteops_security_agent_scan_code_signals( $files ) {
    $patterns = array(
...
function siteops_security_agent_has_high_signal( $signals ) {
