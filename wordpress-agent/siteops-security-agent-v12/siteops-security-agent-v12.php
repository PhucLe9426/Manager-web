<?php
/**
 * Plugin Name: SiteOps Security Agent v1.2
...
define( 'SITEOPS_SECURITY_V12_VERSION', '1.2.0' );
    ...
    static function () {
        register_rest_route(
            ...
            array(
                ...
                'callback'            => 'siteops_security_v12_scan_plugins',
 ...
 */
function siteops_security_v12_scan_plugins() {
    ...
    foreach ( get_plugins() as $plugin_file => $plugin_data ) {
        $results[] = siteops_security_v12_scan_plugin( $plugin_file, $plugin_data );
    }

    $payload = array(
        'agentVersion'     => SITEOPS_SECURITY_V12_VERSION,
    ...
    return rest_ensure_response( siteops_security_v12_json_safe( $payload ) );
}

/**
 * Loại bỏ byte UTF-8 lỗi trong metadata/tên file plugin để WordPress luôn mã hóa được JSON.
 */
function siteops_security_v12_json_safe( $value ) {
    if ( is_array( $value ) ) {
        $safe = array();
        foreach ( $value as $key => $item ) {
            $safe_key          = is_string( $key ) ? wp_check_invalid_utf8( $key, true ) : $key;
            $safe[ $safe_key ] = siteops_security_v12_json_safe( $item );
        }
        return $safe;
    }
...
function siteops_security_v12_scan_plugin( $plugin_file, $plugin_data ) {
    ...
    $files      = '.' === $directory
        ...
        : siteops_security_v12_list_files( $plugin_dir );
    $checksums  = siteops_security_v12_get_checksums( $slug, $version );
    $findings   = array();
    $modified   = array();
    $missing    = array();
    $unexpected = array();
    ...
    $code_signals = 'siteops-security-agent-v12' === $slug ? array() : siteops_security_v12_scan_code_signals( $files );
    if ( $modified ) {
    ...
    $risk      = 'low';
    if ( 'modified' === $integrity || siteops_security_v12_has_high_signal( $code_signals ) ) {
        $risk = 'high';
    } elseif ( $code_signals ) {
        $risk = 'medium';
...
function siteops_security_v12_get_checksums( $slug, $version ) {
    if ( ! $slug || ! $version ) {
        return null;
    }
...
function siteops_security_v12_list_files( $root ) {
    $files = array();
    if ( ! is_dir( $root ) ) {
        return $files;
    }
...
function siteops_security_v12_scan_code_signals( $files ) {
    $patterns = array(
...
function siteops_security_v12_has_high_signal( $signals ) {
